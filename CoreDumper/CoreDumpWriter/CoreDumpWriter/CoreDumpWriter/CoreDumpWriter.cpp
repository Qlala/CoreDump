...
{
	dump_writer* test = cd_CreateFile((char*)"test.set");
	...
	if (fs::is_directory(path)) {
		for (auto & it : fs::directory_iterator(path))
		{
			try {
				...
				std::cout << "error in compute size of Dir :"<< it.path()<<" :" << e.what() << std::endl;
			}
...
int main()
{
	...
	dump_writer* test = Create((char*)"test.set");
	...
	for (i = 0; i < TEST_FRAME_COUNT; i++) {
		...
		AddCycle(test, frame_buff,frame_size);
		...
		int n_change = alter_frame(frame_buff, frame_size, 100, 1,100);
		//if (cdTop_TryWaitSema(test->top)) {
			curr_size = computeSizeOfDir("test/");
			if(std::experimental::filesystem::exists("test.set"))
		...
		stri_st << "Frame:" << i << "/" << TEST_FRAME_COUNT<< "  "<<ti <<" "<<curr_size/1000000000<<"GB"<<" "<<(curr_size/1000000)%1000<<"MB "<< std::setprecision(2) <<(curr_size*100.f / ((i + 1)* cycle_size)) <<"%" << std::endl;
