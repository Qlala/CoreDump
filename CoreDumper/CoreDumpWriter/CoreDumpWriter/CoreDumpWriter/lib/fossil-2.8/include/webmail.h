...
typedef struct EmailToc EmailToc;
EmailToc *emailtoc_from_email(Blob *pEmail);
#define LOCAL static
LOCAL void emailtoc_add_multipart(EmailToc *p,char *z);
int fossil_isalnum(char c);
LOCAL void emailtoc_add_multipart_segment(EmailToc *p,char *z,int bAddHeader);
char *mprintf(const char *zFormat,...);
int fossil_isspace(char c);
void emailtoc_new_header_line(EmailToc *p,char *z);
void *fossil_realloc(void *p,size_t n);
typedef struct EmailBody EmailBody;
