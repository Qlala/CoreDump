...
void hyperlink_to_uuid(const char *zUuid){
  if( g.perm.Hyperlink ){
    cgi_printf("%z[%S]</a>\n",(chref("timelineHistLink","%R/info/%!S",zUuid)),(zUuid));
  }else{
...
void timeline_output_graph_javascript(
  ...
  if( pGraph && pGraph->nErr==0 ){
    ...
    cgi_printf("<script id='timeline-data-%d' type='application/json'>{\n"
           ...
           "  \"scrollToSelect\": %d,\n"
           "  \"nrail\": %d,\n"
           "  \"baseUrl\": \"%R\",\n",(iTableId),(iTableId),(circleNodes),(showArrowheads),(iRailPitch),(colorGraph),(PB("nomo")),(iTopRow),(omitDescenders),(fileDiff),(scrollToSelect),(pGraph->mxRail+1));
    if( pGraph->nRow==0 ){
...
void page_timeline(void){
  ...
  */
  ...
  if( zNewerButton ){
    cgi_printf("%zMore&nbsp;&uarr;</a>\n",(chref("button","%z",zNewerButton)));
  }
  www_print_timeline(&q, tmFlags, zThisUser, zThisTag, selectedRid, 0);
  db_finalize(&q);
  if( zOlderButton ){
    cgi_printf("%zMore&nbsp;&darr;</a>\n",(chref("button","%z",zOlderButton)));
  }
  style_footer();
}
