...
void annotation_page(void){
  ...
  if( !ann.bMoreToDo ){
    ...
    cgi_printf("<h2>Lines of\n"
           ...
           "check-in %z%S</a>:</h2>\n",(href("%R/finfo?name=%h&ci=%!S", zFilename, zCI)),(zFilename),(href("%R/info/%!S",zCI)),(zCI),(href("%R/info/%!S",zOrigin)),(zOrigin));
  }else{
    cgi_printf("<h2>Lines added by the %d most recent ancestors of\n"
           "%z%h</a>\n"
           "from check-in %z%S</a>:</h2>\n",(ann.nVers),(href("%R/finfo?name=%h&ci=%!S", zFilename, zCI)),(zFilename),(href("%R/info/%!S",zCI)),(zCI));
  }
  cgi_printf("<pre>\n");
  szHash = 10;
