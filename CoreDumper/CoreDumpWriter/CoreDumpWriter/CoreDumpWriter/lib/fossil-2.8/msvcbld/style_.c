...
void style_load_one_js_file(const char *zFile){
  cgi_printf("<script src='%R/builtin/%s?id=%S'></script>\n",(zFile),(MANIFEST_UUID));
}
...
static void style_load_all_js_files(void){
  ...
  if( needHrefJs ){
    ...
    cgi_printf("<script id='href-data' type='application/json'>"
  ...
  if( nSubmenu+nSubmenuCtrl>0 ){
    ...
    for(i=0; i<nSubmenuCtrl; i++){
      ...
      switch( aSubmenuCtrl[i].eType ){
        case FF_ENTRY:
          cgi_printf("<span class='submenuctrl%s'>"
                 "&nbsp;%h"
                 "<input type='text' name='%s' value='%h' ",(zXtraClass),(aSubmenuCtrl[i].zLabel),(zQPN),(PD(zQPN, "")));
        ...
        case FF_CHECKBOX: {
          cgi_printf("<label class='submenuctrl submenuckbox%s'>"
                 "<input type='checkbox' name='%s' id='submenuctrl-%d' ",(zXtraClass),(zQPN),(i));
          if( PB(zQPN) ){
            cgi_printf("checked ");
          }
...
void webpage_error(const char *zFormat, ...){
  ...
  if( isAuth ){
    ...
    cgi_printf("g.zBaseURL = %h<br />\n"
           ...
           "g.userUid = %d<br />\n"
           "g.zLogin = %h<br />\n"
           "g.isHuman = %d<br />\n",(g.zBaseURL),(g.zHttpsURL),(g.zTop),(g.zPath),(g.userUid),(g.zLogin),(g.isHuman));
    if( g.nRequest ){
    ...
    cgi_printf("g.zRepositoryName = %h<br />\n"
           "load_average() = %f<br />\n"
           "cgi_csrf_safe(0) = %d<br />\n"
           "<hr />\n",(g.zRepositoryName),(load_average()),(cgi_csrf_safe(0)));
    P("HTTP_USER_AGENT");
