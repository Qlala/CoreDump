...
void wikiedit_page(void){
  ...
  if( !isWysiwyg ){
    ...
    cgi_printf("<br /><textarea name=\"w\" class=\"wikiedit\" cols=\"80\" "
           " rows=\"%d\" wrap=\"virtual\" placeholder=\"%h\">"
           "%h</textarea>\n"
           "<br />\n",(n),(zPlaceholder),(zBody));
  ...
  if( havePreview ){
    if( zBody[0] ){
      ...
      cgi_printf("<input type=\"submit\" name=\"submit\" value=\"Delete This Wiki Page\" />\n");
    }
  }
...
void wcontent_page(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( wrid==0 ){
      ...
      cgi_printf("<tr><td data=sortkey='%h\">"
             "%z%h</a></td>\n",(zSort),(href("%R/wiki?name=%T",zWName)),(zWDisplayName));
    }
...
static void wiki_section_label(
  ...
  if( (mFlags & WIKIASSOC_FULL_TITLE)==0 ){
    cgi_printf("<div class=\"section\">About</div>\n");
  }else if( zPrefix[0]=='c' ){  /* checkin/... */
    cgi_printf("<div class=\"section\">About checkin %.20h</div>\n",(zName));
  }else{
    cgi_printf("<div class=\"section\">About %s %h</div>\n",(zPrefix),(zName));
  }
}
...
int wiki_render_associated(
  ...
  if( fossil_strcmp(pWiki->zMimetype, "text/x-markdown")==0 ){
    ...
    if( blob_size(&title) ){
      cgi_printf("<div class=\"section\">%h</div>\n",(blob_str(&title)));
    }else{
