...
void setup_notification(void){
  ...
  entry_attribute("Store Emails In This Database", 60, "email-send-db",
                   "esdb", "", 0);
  cgi_printf("<p>When the send method is \"store in a databaes\", each email message is\n"
...
void subscribe_page(void){
  ...
  if( needCaptcha ){
    ...
    cgi_printf("<tr>\n"
           ...
           " <input type=\"hidden\" name=\"captchaseed\" value=\"%u\"></td>\n"
           "</tr>\n",(uSeed));
    if( eErr==2 ){
      cgi_printf("<tr><td><td><span class='loginError'>&uarr; %h</span></td></tr>\n",(zErr));
    }
...
static void alert_unsubscribe(const char *zName){
  ...
  if( zEmail==0 ){
    ...
    cgi_printf("<p>The \"%h\" email address has been delisted.\n"
...
void alert_page(void){
  ...
  if( g.perm.Admin ){
    cgi_printf("<tr>\n"
           ...
           " <td><input type=\"text\" name=\"suname\" value=\"%h\" "
           " size=\"30\"></td>\n"
           "</tr>\n",(sctime),(mtime),(smip),(suname?suname:""));
  }
  cgi_printf("<tr>\n"
         " <td class=\"form_label\">Topics:</td>\n"
         " <td><label><input type=\"checkbox\" name=\"sa\" %s>"
...
#if 0
  cgi_printf(" <label><input type=\"checkbox\" name=\"sdigest\" %s>"
...
void subscriber_list_page(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    cgi_printf("<tr>\n"
           ...
           "<td>%h</td>\n"
           "</tr>\n",(db_column_text(&q,0)),(db_column_text(&q,1)),(db_column_text(&q,2)),(db_column_int(&q,5)?"digest":""),(db_column_text(&q,3)),(db_column_int(&q,4)?"yes":"pending"),(iMtime),(human_readable_age(rAge)),(db_column_text(&q,7)));
  }
...
void announce_page(void){
  ...
  if( fossil_strcmp(P("name"),"test1")==0 ){
    ...
    if( zErr ){
      ...
      cgi_printf("<p>The announcement has been sent.</p>\n");
    }
    style_footer();    
    return;
