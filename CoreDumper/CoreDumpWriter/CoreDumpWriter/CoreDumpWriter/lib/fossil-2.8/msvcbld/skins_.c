...
static int skinRename(void){
  ...
  if( zNewName==0 || zNewName[0]==0 || (ex = skinExists(zNewName))!=0 ){
    ...
    cgi_printf("<form action=\"%s/setup_skin_admin\" method=\"post\"><div>\n"
           "<table border=\"0\"><tr>\n"
...
void setup_skin_admin(void){
  ...
  for(i=0; i<count(aBuiltinSkin); i++){
    ...
    if( fossil_strcmp(aBuiltinSkin[i].zSQL, zCurrent)==0 ){
      ...
      cgi_printf("<form action=\"%s/setup_skin_admin\" method=\"post\">\n"
             "<input type=\"hidden\" name=\"sn\" value=\"%h\" />\n"
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( once ){
      once = 0;
      cgi_printf("<tr><td colspan=4><h2>Skins saved as \"skin:*' entries "
             "in the CONFIG table:</h2></td></tr>\n");
    }
    cgi_printf("<tr><td>%d.<td>%h<td>&nbsp;&nbsp;<td>\n"
           "<form action=\"%s/setup_skin_admin\" method=\"post\">\n",(i),(zN),(g.zTop));
  ...
  if( !seenCurrent ){
    i++;
    cgi_printf("<tr><td colspan=4><h2>Current skin in css/header/footer/details entries "
           "in the CONFIG table:</h2></td></tr>\n"
           "<tr><td>%d.<td><i>Current</i><td>&nbsp;&nbsp;<td>\n"
           "<form action=\"%s/setup_skin_admin\" method=\"post\">\n"
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( once ){
      once = 0;
      cgi_printf("<tr><td colspan=4><h2>Draft skins stored as \"draft[1-9]-*' entries "
             "in the CONFIG table:</h2></td></tr>\n");
    }
    cgi_printf("<tr><td>%d.<td>%h<td>&nbsp;&nbsp;<td>\n"
           "<form action=\"%s/setup_skin_admin\" method=\"post\">\n"
...
static void skin_emit_skin_selector(
  ...
  if( fossil_strcmp(zExcept, "current")!=0 ){
    cgi_printf("<option value='current'>Currently In Use</option>\n");
  }
  for(i=0; i<count(aBuiltinSkin); i++){
...
void setup_skinedit(void){
  ...
  cgi_printf("<form action=\"%s/setup_skinedit\" method=\"post\"><div>\n",(g.zTop));
  login_insert_csrf_secret();
