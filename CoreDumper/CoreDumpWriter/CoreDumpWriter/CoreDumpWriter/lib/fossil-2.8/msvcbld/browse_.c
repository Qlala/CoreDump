...
void page_dir(void){
  ...
  */
  ...
  if( zCI ){
    ...
    if( zD ){
      cgi_printf("&nbsp;&nbsp;%z[history]</a>\n",(href("%R/timeline?chng=%T/*", zD)));
    }
    cgi_printf("</h2>\n");
  ...
  */
  ...
  cgi_printf("<div class=\"columns\" style=\"columns: %dex %d;\">\n"
         "<ul class=\"browser\">\n",(mxLen),(n));
...
void fileage_page(void){
  ...
  while( db_step(&q1)==SQLITE_ROW ){
    ...
    cgi_printf("</td>\n"
           "<td>\n"
           "%W\n"
           "(check-in:&nbsp;%z%S</a>,\n",(zComment),(href("%R/ci/%!S",zUuid)),(zUuid));
    if( showId ){
      cgi_printf("id: %d\n",(mid));
    }
