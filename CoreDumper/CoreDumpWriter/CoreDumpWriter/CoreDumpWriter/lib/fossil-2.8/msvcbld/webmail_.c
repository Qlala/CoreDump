    ...
    if( eFormat==2 ){
      ...
      for(i=0; i<p->nHdr; i++){
        ...
        if( z[j]!=':' ){
          ...
          z[j] = 0;
          cgi_printf("<b>%h:</b> %h<br>\n",(z),(z+j+1));
        }
      }
      for(i=0; i<p->nBody; i++){
        cgi_printf("<hr><b>Messsage Body #%d: %h ",(i),(p->aBody[i].zMimetype));
        if( p->aBody[i].zFilename ){
          cgi_printf("\"%h\"\n",(p->aBody[i].zFilename));
        }
        cgi_printf("</b>\n");
...
void webmail_page(void){
  ...
  if( !db_table_exists("repository","emailbox") ){
    ...
    cgi_printf("<p>This repository is not configured to provide webmail</p>\n");
    style_footer();
    return;
  }
  ...
  blob_reset(&sql);
  cgi_printf("<form action=\"%R/webmail\" method=\"POST\">\n"
         "<input type=\"hidden\" name=\"d\" value=\"%d\">\n"
         "<input type=\"hidden\" name=\"user\" value=\"%h\">\n"
         "<table border=\"0\" width=\"100%%\">\n"
         "<tr><td align=\"left\">\n",(d),(zUser?zUser:"*"));
  if( d==2 ){
    cgi_printf("<input type=\"submit\" name=\"read\" value=\"Undelete\">\n"
           "<input type=\"submit\" name=\"purge\" value=\"Delete Permanently\">\n");
  }else{
    cgi_printf("<input type=\"submit\" name=\"trash\" value=\"Delete\">\n");
    if( d!=1 ){
      cgi_printf("<input type=\"submit\" name=\"unread\" value=\"Mark as unread\">\n");
    }
    cgi_printf("<input type=\"submit\" name=\"read\" value=\"Mark as read\">\n");
  }
  cgi_printf("<button onclick=\"webmailSelectAll(); return false;\">Select All</button>\n"
         "<a href=\"%h\">refresh</a>\n"
         "</td><td align=\"right\">\n",(url_render(&url,0,0,0,0)));
  if( pg>0 ){
    sqlite3_snprintf(sizeof(zPPg), zPPg, "%d", pg-1);
    cgi_printf("<a href=\"%s\">&lt; Newer</a>&nbsp;&nbsp;\n",(url_render(&url,"pg",zPPg,0,0)));
  }
  if( got>50 ){
    sqlite3_snprintf(sizeof(zNPg),zNPg,"%d",pg+1);
    cgi_printf("<a href=\"%s\">Older &gt;</a></td>\n",(url_render(&url,"pg",zNPg,0,0)));
  }
  cgi_printf("</table>\n"
         "<table>\n");
  while( db_step(&q)==SQLITE_ROW ){
    ...
    cgi_printf("<tr>\n"
           "<td><input type=\"checkbox\" class=\"webmailckbox\" name=\"e%s\"></td>\n"
           "<td>%h</td>\n"
           "<td><a href=\"%h\">%h</a> "
           "%s</td>\n",(zId),(zFrom),(url_render(&url,"id",zId,0,0)),(zSubject),(zDate));
    if( showAll ){
      const char *zTo = db_column_text(&q,5);
      cgi_printf("<td><a href=\"%h\">%h</a></td>\n",(url_render(&url,"user",zTo,0,0)),(zTo));
    }
    cgi_printf("</tr>\n");
  }
  db_finalize(&q);
...
void webmail_emailblob_page(void){
  ...
  if( id>0 ){
    ...
    while( db_step(&q)==SQLITE_ROW ){
      ...
      cgi_printf("<li> <a href=\"%R/emailblob?id=%d\">emailblob entry %d</a>\n",(id),(id));
    }
    db_finalize(&q);
    ...
    while( db_step(&q)==SQLITE_ROW ){
      ...
      cgi_printf("<li> emailbox for %h state %d\n",(zUser),(e));
    }
    db_finalize(&q);
    ...
    while( db_step(&q)==SQLITE_ROW ){
      ...
      cgi_printf("<li> emailoutq message body from %h to %h\n",(zFrom),(zTo));
    }
    db_finalize(&q);
    ...
    while( db_step(&q)==SQLITE_ROW ){
      const char *zFrom = db_column_text(&q, 0);
      const char *zTo = db_column_text(&q, 1);
      cgi_printf("<li> emailoutq transcript from %h to %h\n",(zFrom),(zTo));
    }
    db_finalize(&q);
    cgi_printf("</ul>\n"
           "<hr>\n");
    ...
    cgi_printf("<table border=\"1\" cellpadding=\"5\" cellspacing=\"0\" class=\"sortable\" "
    ...
    while( db_step(&q)==SQLITE_ROW ){
      ...
      cgi_printf("<tr>\n"
             " <td align=\"right\"><a href=\"%R/emailblob?id=%d\">%d</a>\n"
             " <td align=\"right\">%d</td>\n",(id),(id),(nref));
      if( ets>0 ){
      ...
      cgi_printf(" <td>%h</td>\n"
             " <td align=\"right\" data-sortkey='%08x'>%,d</td>\n"
             " <td align=\"right\" data-sortkey='%08x'>%,d</td>\n"
             "</tr>\n",(zDate),(sz),(sz),(csz),(csz));
    }
...
void webmail_emailoutq_page(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    cgi_printf("<tr>\n"
           ...
           " <td>%h\n"
           " <td>%h\n"
           " <td align=\"right\">%d\n",(zDomain),(zFrom),(zTo),(emsgid),(emsgid),(zCTime),(zMTime),(ensend));
    if( ets>0 ){
      cgi_printf(" <td align=\"right\"><a href=\"%R/emailblob?id=%d\">%d</a></td>\n",(ets),(ets));
    }else{
      cgi_printf(" <td>&nbsp;</td>\n");
    }
  }
