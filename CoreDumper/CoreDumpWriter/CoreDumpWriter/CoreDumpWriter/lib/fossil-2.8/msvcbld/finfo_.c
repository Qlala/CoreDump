...
void finfo_page(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    cgi_printf("<td class=\"timelineTime\">"
           ...
           "</td>\n",(href("%R/artifact/%!S",zUuid)),(zTime),(gidx));
    if( zBgClr && zBgClr[0] ){
    ...
    if( zUuid && (tmFlags & TIMELINE_VERBOSE)==0 ){
      cgi_printf("file:&nbsp;%z[%S]</a>\n",(href("%R/artifact/%!S",zUuid)),(zUuid));
      if( fShowId ){
        int srcId = delta_source_rid(frid);
        if( srcId>0 ){
          cgi_printf("id:&nbsp;%d&larr;%d\n",(frid),(srcId));
        }else{
          cgi_printf("id:&nbsp;%d\n",(frid));
        }
      }
    ...
    if( g.perm.Hyperlink && zUuid ){
      ...
      cgi_printf("<span id='links-%d'><span class='timelineExtraLinks'>\n"
             ...
             "%z\n"
             "[blame]</a>\n"
             "%z[check-ins&nbsp;using]</a>\n",(frid),(href("%R/annotate?filename=%h&checkin=%s",z,zCkin)),(href("%R/blame?filename=%h&checkin=%s",z,zCkin)),(href("%R/timeline?n=all&uf=%!S",zUuid)));
      if( fpid>0 ){
